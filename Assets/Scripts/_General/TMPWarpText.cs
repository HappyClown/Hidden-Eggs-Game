...
public class TMPWarpText : MonoBehaviour {
	[Header ("Triggers")]
	public bool warpOnce;
	...
	void Update () {
		if (StartAnimatedWarp) {
			StartCoroutine(ContinuousWarpText());
			StartAnimatedWarp = false;
		}
		if (warpOnce) {
			WarpText();
			warpOnce = false;
	...
	/// <returns></returns>
	IEnumerator ContinuousWarpText()
	{
		warping = true;
		VertexCurve.preWrapMode = WrapMode.Clamp;
		VertexCurve.postWrapMode = WrapMode.Clamp;
